...
function applyTranslations(language) {
  // === NEW: HANDLE TITLE + META TAGS (SEO MAGIC) ===
  if (translations[language]) {
    // Update TITLE tag
    const titleEl = document.querySelector('title[data-translate-title]');
    if (titleEl && translations[language].aiRenderTitle) {
      titleEl.textContent = translations[language].aiRenderTitle;
    }
    
    // Update META description
    const metaDesc = document.querySelector('meta[name="description"][data-translate]');
    if (metaDesc && translations[language].aiRenderShortDesc) {
      metaDesc.setAttribute('content', translations[language].aiRenderShortDesc);
    }
    
    // Update OG tags
    const ogTitle = document.querySelector('meta[property="og:title"][data-translate]');
    if (ogTitle && translations[language].aiRenderTitle) {
      ogTitle.setAttribute('content', translations[language].aiRenderTitle);
    }
    
    const ogDesc = document.querySelector('meta[property="og:description"][data-translate]');
    if (ogDesc && translations[language].aiRenderShortDesc) {
      ogDesc.setAttribute('content', translations[language].aiRenderShortDesc);
    }
  }

  // === YOUR EXISTING CODE (UNCHANGED - PERFECT!) ===
  const elements = document.querySelectorAll("[data-translate], [data-placeholder]");
  elements.forEach((element) => {
    const translationKey = element.getAttribute("data-translate") || element.getAttribute("data-placeholder");

    if (translations[language] && translations[language][translationKey]) {
      let translatedText = translations[language][translationKey];
      translatedText = translatedText.replace(/\n/g, "<br>");

      if ((element.tagName === "INPUT" || element.tagName === "TEXTAREA") && element.hasAttribute("data-placeholder")) {
        element.setAttribute("placeholder", translatedText);
      }
      else if (element.tagName === "SELECT" && element.hasAttribute("data-placeholder")) {
